...
class AppStore extends Component {
  ...
  render() {
    ...
    const filteredAppsList = this.getFilteredApps(searchResults)

    return (
      <div className="app-store-bg-cont">
        <h1>App Store</h1>
        ...
        <ul className="tab-items-wrapper">
          {tabsList.map(tab => (
            <TabItem
              key={tab.tabId}
              tabDetails={tab}
              isActive={activeTabId === tab.tabId}
              onClickTab={this.updateTabId}
            />
          ))}
        </ul>
